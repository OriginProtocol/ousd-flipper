...
export async function setApproval(stable) {
    const coin = new ethers.Contract(ADDRESSES[stable], ousd_abi, provider.getSigner())
    const desired = ethers.utils.parseEther((10 * 1e10).toString())
    console.log("Creating approval", stable, ADDRESSES[stable], ADDRESSES.FLIPPER, desired.toString())
    let tx
    if(stable=="DAI" || stable=="USDT"){
        tx = await coin.approve(ADDRESSES.FLIPPER, desired.toString(), {gasLimit:40000})
    } else {
        tx = await coin.increaseAllowance(ADDRESSES.FLIPPER, desired.toString(), {gasLimit:40000})
    }
    console.log(tx)
...
export async function updateEthPrice() {
    const chainlink = new ethers.Contract(ADDRESSES.CHAINLINK_ETH_USD, chainlink_abi, provider)
